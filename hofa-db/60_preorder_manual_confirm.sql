-- Đơn đặt trước/giá sỉ (sales_model='scheduled') giờ gửi thẳng về cửa hàng xác nhận NGAY lúc
-- đặt (xem 59_preorder_confirm_now.sql — orderOffer.js không còn giữ đơn "ngủ" nữa), nhưng màn
-- xác nhận (store app) PHẢI luôn ở chế độ thủ công (thanh trượt dài, hết giờ chỉ huỷ đơn chứ
-- không đóng chi nhánh — xem order_detail_screen.dart) BẤT KỂ chi nhánh có bật "Tự động nhận
-- đơn" hay không — quyết định lùi thời hạn đặt trước vài ngày không nên tự động chốt như đơn
-- tức thời. Ép ngay từ lúc chốt confirm_sweep_deadline trong create_order() để client không
-- phải tự đoán, chỉ cần đọc y nguyên deadline đã chốt sẵn từ server.
-- Chữ ký hàm giữ nguyên, thân hàm giữ nguyên phần còn lại y hệt hofa-db/50_voucher_conditions.sql,
-- chỉ đổi 1 chỗ duy nhất: v_branch_auto_accept ép về false khi p_sales_model = 'scheduled'.
