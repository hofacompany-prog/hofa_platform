-- ============================================================================
-- MIGRATION 81 — Cửa hàng bật/tắt từng lựa chọn topping mà không cần xoá (VD: hết trân châu
-- đen hôm nay) — khách không còn thấy lựa chọn đã tắt, nhưng cửa hàng/admin vẫn xem/bật lại
-- được bất cứ lúc nào. sort_order của topping_groups/product_toppings đã có sẵn từ migration
-- 08/17 nhưng chưa có UI kéo-thả sắp xếp — phần đó sửa ở app, không cần đổi DB.
-- ============================================================================

ALTER TABLE product_toppings ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT true;
COMMENT ON COLUMN product_toppings.is_active IS
  'Cửa hàng tạm ẩn lựa chọn này khỏi khách (hết hàng...) mà không xoá — vẫn giữ lịch sử đơn cũ đã chọn topping này';

-- Thân hàm giữ nguyên y hệt hofa-db/77_buy_on_behalf_unlimited_stock.sql, chỉ thêm điều kiện
-- pt.is_active vào subquery chốt topping — tránh chốt giá 1 topping khách đã lỡ chọn (cache
-- cũ trên máy) đúng lúc cửa hàng vừa tắt lựa chọn đó.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- chỉ nhận topping_id rồi tự tra giá thật từ product_toppings. Bỏ qua topping đã bị
    -- cửa hàng tắt (is_active=false) — xem hofa-db/81_topping_active_toggle.sql.
    v_topping_sum := 0;
    v_toppings_json := '[]'::jsonb;
...
COMMENT ON FUNCTION create_order IS
  'Tạo đơn: chốt giá + giữ tồn kho từng món (BỎ QUA giữ chỗ tồn kho nếu cửa hàng mua hộ, BỎ QUA topping đã bị tắt), áp voucher, chốt hoa hồng/VAT/TNCN, insert orders + order_items trong 1 transaction.';
