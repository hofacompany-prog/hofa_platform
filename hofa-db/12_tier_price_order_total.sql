-- ============================================================================
-- MIGRATION 12 — Bậc giá "đặt trước" chốt theo TỔNG số phần cả đơn (gộp mọi sản
-- phẩm), không chỉ riêng số lượng của từng món
--
-- Trước giờ resolve_variant_price() chỉ so số lượng của RIÊNG 1 dòng (1 sản phẩm) với
-- min_quantity/max_quantity của bậc giá — đúng cho bậc "giá sỉ" (lead_time_days = 0,
-- mua càng nhiều 1 món giá càng tốt), nhưng sai cho bậc "đặt trước" (lead_time_days > 0):
-- cửa hàng muốn tính theo TỔNG số phần khách đặt trong CÙNG 1 lần giao, không phân biệt
-- đặt món gì (vd đặt 4 món khác nhau, mỗi món 3 phần, tổng 12 phần vẫn đạt bậc "từ 10").
--
-- resolve_variant_price() nhận thêm p_order_quantity (tổng số lượng của CẢ đơn, cộng dồn
-- mọi món) — bậc có lead_time_days = 0 vẫn so theo p_quantity (số lượng riêng món đó),
-- bậc có lead_time_days > 0 so theo p_order_quantity (tổng cả đơn).
--
-- Điều kiện để đúng: 1 đơn hàng phải CHỈ chứa đúng những món giao cùng 1 ngày (app đã
-- sửa lại checkout để tách mỗi lần giao thành 1 đơn riêng thay vì gộp hết mọi món của
-- giỏ "đặt trước" vào 1 đơn cho mỗi lần giao) — nếu không, "tổng cả đơn" sẽ lẫn cả món
-- của ngày khác.
--
...
DROP FUNCTION IF EXISTS resolve_variant_price(UUID, INTEGER, TIMESTAMPTZ, INTEGER);
