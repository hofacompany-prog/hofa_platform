-- ============================================================================
-- MIGRATION 13 — Bậc "đặt trước" có 2 điều kiện độc lập: số lượng tổng phần trong cùng
-- 1 lần giao, VÀ số ngày/tuần khách đặt RIÊNG sản phẩm đó — mỗi tổ hợp 1 giá riêng
--
-- Trước giờ cột lead_time_days trên bậc "đặt trước" nghĩa là "đặt trước tối thiểu bao
-- nhiêu ngày mới được giao" (so với p_scheduled_for). Đổi lại: cột này (đổi tên thành
-- min_days_per_week) giờ nghĩa là "khách phải đặt sản phẩm này tối thiểu bao nhiêu
-- ngày/tuần mới đạt điều kiện ngày" — độc lập với điều kiện số lượng (min_quantity/
-- max_quantity, tính theo TỔNG số phần cả lần giao, xem migration 12).
--
-- Mỗi bậc "đặt trước" giờ có 3 giá tuỳ theo đạt điều kiện nào:
--   unit_price       — chỉ đạt điều kiện số lượng
--   unit_price_days  — chỉ đạt điều kiện số ngày/tuần (MỚI)
--   unit_price_both  — đạt cả 2 điều kiện (MỚI, thường rẻ nhất)
-- Bậc "giá sỉ" (min_days_per_week = 0) không đổi — vẫn 1 giá duy nhất (unit_price), chỉ
-- điều kiện số lượng.
--
-- resolve_variant_price() nhận p_days_count thay cho p_scheduled_for (số ngày/tuần
-- RIÊNG sản phẩm này, client tính từ số ngày trong tuần đã tick cho món đó rồi gửi kèm
-- theo từng item lúc tạo đơn — field "days_count" trong p_items).
--
-- Chạy 1 lần trên Supabase SQL Editor. Đã gộp vào hofa-db/01_schema.sql (bảng) và
-- hofa-db/04_api_functions.sql (hàm) cho lần dựng DB mới từ đầu.
-- ============================================================================

BEGIN;

ALTER TABLE wholesale_tiers RENAME COLUMN lead_time_days TO min_days_per_week;
...
DROP FUNCTION IF EXISTS resolve_variant_price(UUID, INTEGER, INTEGER, TIMESTAMPTZ, INTEGER);
