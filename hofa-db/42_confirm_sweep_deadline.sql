-- Mốc giờ hết hạn thanh trượt xác nhận ở màn chi tiết đơn (store app) — chốt NGAY LÚC TẠO ĐƠN
-- (server), y hệt cách driver app đã làm với deliveries.accept_deadline (xem dispatch.js). Lý
-- do: trước đây thời lượng thanh chạy màu chỉ tồn tại phía CLIENT (AnimationController khởi
-- tạo mới mỗi lần màn hình mở) — thoát app rồi vào lại đúng đơn đó làm thanh chạy LẠI TỪ ĐẦU dù
-- thực tế đã qua bao lâu. Có mốc giờ cố định ở server thì client chỉ cần tính lại
-- "còn bao nhiêu giây tới deadline" mỗi lần mở màn, không bao giờ reset.
ALTER TABLE orders ADD COLUMN confirm_sweep_deadline TIMESTAMPTZ;
COMMENT ON COLUMN orders.confirm_sweep_deadline IS
  'Mốc giờ hết hạn thanh trượt xác nhận ở màn chi tiết đơn (store app) — chốt NGAY LÚC TẠO ĐƠN (create_order()) theo auto_accept_settings.confirm_sweep_seconds (chi nhánh BẬT "Tự động nhận đơn") hoặc manual_confirm_sweep_seconds (TẮT), tuỳ branches.auto_accept_orders của chi nhánh lúc đặt đơn. Đóng/mở lại màn hình chỉ tính lại thời gian còn LẠI tới đúng mốc này, không reset. Đơn tạo trước migration này có giá trị NULL.';

-- create_order(): thêm bước tự tính confirm_sweep_deadline ngay trước khi insert orders — chữ
-- ký hàm giữ nguyên, không thêm tham số vì mốc này KHÔNG do client quyết định.
