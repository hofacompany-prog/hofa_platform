-- Thêm 4 điều kiện áp dụng voucher: loại đơn, loại cửa hàng, số đơn khách đang có cùng lúc
-- (chưa tới trạng thái delivering), và bậc giảm giá theo giá trị đơn — tất cả validate ở
-- create_order() (server không bao giờ tin client cho bất cứ gì ảnh hưởng tới giá).

ALTER TABLE orders ADD COLUMN order_kind TEXT;
ALTER TABLE orders ADD CONSTRAINT orders_order_kind_valid
  CHECK (order_kind IS NULL OR order_kind IN ('instant', 'preorder', 'wholesale'));
COMMENT ON COLUMN orders.order_kind IS
  'instant | preorder | wholesale — chốt lúc tạo đơn (create_order()) từ order_kind của các món trong giỏ (mọi món trong 1 đơn luôn cùng loại, xem CartItem app khách), instant nếu sales_model=instant. Dùng để lọc điều kiện voucher theo loại đơn (vouchers.applicable_order_kinds). NULL với đơn tạo trước migration này.';

ALTER TABLE vouchers
  ADD COLUMN applicable_order_kinds TEXT[],
  ADD COLUMN applicable_merchant_types TEXT[],
  ADD COLUMN min_concurrent_orders INTEGER;
ALTER TABLE vouchers ADD CONSTRAINT vouchers_order_kinds_valid
  CHECK (applicable_order_kinds IS NULL OR applicable_order_kinds <@ ARRAY['instant', 'preorder', 'wholesale']);
ALTER TABLE vouchers ADD CONSTRAINT vouchers_merchant_types_valid
  CHECK (applicable_merchant_types IS NULL OR applicable_merchant_types <@ ARRAY['standard', 'regular', 'wholesale', 'buy_on_behalf']);
ALTER TABLE vouchers ADD CONSTRAINT vouchers_min_concurrent_valid
  CHECK (min_concurrent_orders IS NULL OR min_concurrent_orders > 0);
COMMENT ON COLUMN vouchers.applicable_order_kinds IS
  'NULL = áp dụng mọi loại đơn. Có giá trị thì đơn phải có orders.order_kind nằm trong mảng này mới dùng được mã.';
COMMENT ON COLUMN vouchers.applicable_merchant_types IS
  'NULL = áp dụng mọi loại cửa hàng. Có giá trị thì merchants.merchant_type của cửa hàng đặt đơn phải nằm trong mảng này.';
COMMENT ON COLUMN vouchers.min_concurrent_orders IS
  'NULL = không yêu cầu. Có giá trị N thì khách phải đang có ÍT NHẤT N đơn (TÍNH CẢ đơn đang tạo) ở trạng thái chưa tới delivering mới dùng được mã — tự "hết hạn" khi 1 đơn được tài xế xác nhận đang giao vì lúc đó không còn tính vào nữa, không cần lưu trạng thái riêng.';

CREATE TABLE voucher_amount_tiers (
  ...
  CONSTRAINT voucher_amount_tiers_min_valid CHECK (min_order_amount >= 0),
  CONSTRAINT voucher_amount_tiers_discount_valid CHECK (discount_value >= 0),
  UNIQUE (voucher_id, min_order_amount)
);
COMMENT ON TABLE voucher_amount_tiers IS
  'Bậc giảm giá theo giá trị đơn cho 1 voucher — chọn bậc có min_order_amount cao nhất mà đơn đạt được, dùng discount_value/max_discount của bậc đó thay cho vouchers.discount_value/max_discount (% nếu voucher.discount_type=percent, VNĐ nếu fixed — không áp dụng cho free_shipping). Voucher không có bậc nào thì dùng nguyên giá trị giảm cố định như trước, tương thích ngược hoàn toàn.';

-- create_order(): thêm order_kind (chốt lúc tạo đơn) + 3 điều kiện voucher mới (loại đơn,
-- loại cửa hàng, số đơn cùng lúc) + tra bậc giảm giá theo giá trị đơn — chữ ký hàm giữ
-- nguyên, thân hàm giữ nguyên phần còn lại y hệt hofa-db/42_confirm_sweep_deadline.sql.
...
BEGIN
  ...
  -- branches.auto_accept_orders của chi nhánh này.
...
COMMENT ON FUNCTION create_order IS
  'Tạo đơn: chốt giá qua resolve_variant_price (bậc giá sỉ theo số lượng riêng món; bậc đặt trước theo tổng số lượng cả đơn + số ngày/tuần cả đơn không phân biệt món, chỉ xét đúng loại bậc theo order_kind của từng món; không khớp thì giá gốc), giữ chỗ tồn kho, áp NHIỀU voucher cùng lúc (tối đa voucher_settings.max_vouchers_per_order, mỗi mã kiểm đủ 6 điều kiện: còn hạn/còn lượt/đạt giá trị tối thiểu/đúng loại đơn/đúng loại cửa hàng/đủ số đơn cùng lúc, rồi tự tra bậc giảm giá theo giá trị đơn nếu voucher có cấu hình voucher_amount_tiers), tự tính phí mua hộ theo merchant_fee_tiers nếu merchant_type = buy_on_behalf, tự chốt order_kind + default_prep_minutes + confirm_sweep_deadline — tất cả trong 1 transaction';
