-- ============================================================================
-- MIGRATION 73 — Ngưỡng giá trị đơn để BẮT BUỘC xác nhận OTP (cửa hàng-tài xế lúc lấy hàng,
-- tài xế-khách lúc giao hàng). Đơn giá trị THẤP HƠN HOẶC BẰNG ngưỡng này bỏ qua xác nhận OTP
-- hoàn toàn (không cần đúng mã, kể cả để trống). Mặc định 0 = mọi đơn thật (total_amount > 0)
-- đều vẫn bắt buộc OTP như trước — hành vi cũ giữ nguyên cho tới khi admin chủ động đặt ngưỡng
-- khác 0. Không đụng gì tới đơn mua hộ (buy_on_behalf) — bước lấy hàng của loại đơn đó vẫn luôn
-- bắt buộc ảnh hoá đơn/hàng đã mua, không liên quan OTP.
-- ============================================================================
...
COMMENT ON TABLE otp_settings IS
  'Ngưỡng giá trị đơn (VNĐ) để bắt buộc xác nhận OTP lấy hàng/giao hàng — đơn <= ngưỡng bỏ qua OTP. Chỉ giữ 1 dòng (mới nhất theo updated_at) đang áp dụng, admin sửa qua GET/PATCH /otp-settings.';

INSERT INTO otp_settings (min_order_amount) VALUES (0);

-- update_delivery_status(): chữ ký + toàn bộ thân hàm giữ nguyên hofa-db/72_driver_tax_rates.sql,
-- chỉ thêm ngưỡng OTP vào 2 chỗ kiểm p_otp (picked_up của đơn thường, delivered).
...
COMMENT ON FUNCTION update_delivery_status IS
  'Đổi trạng thái chuyến giao, kiểm OTP CHỈ khi đơn vượt ngưỡng otp_settings (trừ picked_up của đơn mua hộ — luôn dùng ảnh thay OTP), đồng bộ trạng thái đơn, trừ kho thật lúc lấy hàng, cộng/trừ ví tài xế + lưu breakdown hoa hồng/thuế GTGT/TNCN lúc giao xong — xem hofa-db/73_otp_threshold_settings.sql.';
