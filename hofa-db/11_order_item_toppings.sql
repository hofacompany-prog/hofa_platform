-- ============================================================================
-- MIGRATION 11 — Cho khách chọn topping khi đặt hàng
--
-- Trước giờ product_topping_groups/product_toppings chỉ là danh mục cho cửa hàng
-- khai báo, chưa có chỗ nào lưu khách đã CHỌN topping gì cho 1 món trong đơn.
-- Thêm bảng order_item_toppings (bản chụp tên/giá lúc đặt, giống order_items chụp
-- lại tên/giá sản phẩm) và sửa create_order() để: nhận topping_ids theo từng item,
-- tự tra giá thật (không tin giá client), cộng vào unit_price, rồi ghi lại đã chọn
-- topping gì.
--
...
-- p_items: [{"variant_id":"uuid","quantity":2,"note":"...","topping_ids":["uuid",...]}, ...]
...
COMMENT ON FUNCTION create_order IS
  'Tạo đơn: chốt giá từ product_variants + topping (không tin giá client gửi), giữ chỗ tồn kho, áp voucher — tất cả trong 1 transaction';
