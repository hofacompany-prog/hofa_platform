-- ============================================================================
-- MIGRATION 67 — Ví cửa hàng cộng đúng THU NHẬP RÒNG (đã trừ VAT/TNCN), không chỉ trừ hoa hồng
-- nữa — theo yêu cầu người dùng: trước đây "Thu nhập" (tab Tóm tắt, trừ cả VAT+TNCN ước tính)
-- và "Số tiền thu về" (tab ví, chỉ trừ hoa hồng) là 2 con số khác nhau, gây nhầm lẫn.
--
-- Cách làm: chốt vat_amount/pit_amount lúc TẠO ĐƠN — cùng lúc, cùng cách với commission_amount
-- đã chốt sẵn từ trước (dùng vat_rate/pit_rate của cửa hàng TẠI THỜI ĐIỂM đặt, không đổi theo
-- nếu cửa hàng đổi tỷ lệ về sau — nhất quán với lý do đã chốt commission_amount, xem
-- hofa-db/64_merchant_wallet_ledger.sql). merchant_payout đổi công thức thành
-- subtotal - commission_amount - vat_amount - pit_amount — số ĐÃ LÀ thu nhập ròng, nên
-- update_order_status (không đổi gì, xem 64_merchant_wallet_ledger.sql) tự động cộng đúng số
-- ròng vào merchant_wallet_transactions khi đơn delivered, không cần sửa hàm đó.
--
-- Đơn cũ tạo TRƯỚC migration này giữ nguyên merchant_payout đã chốt (chỉ trừ hoa hồng) — không
-- hồi tố, đúng nguyên tắc không sửa số đã ghi sổ cái.
-- ============================================================================

ALTER TABLE orders ADD COLUMN vat_amount INTEGER NOT NULL DEFAULT 0;
ALTER TABLE orders ADD COLUMN pit_amount INTEGER NOT NULL DEFAULT 0;
COMMENT ON COLUMN orders.vat_amount IS
  'Thuế GTGT ước tính của cửa hàng trên subtotal đơn này — chốt lúc tạo đơn theo merchants.vat_rate tại thời điểm đó, trừ luôn vào merchant_payout (khác tax_amount: đó là thuế phía khách, không liên quan).';
COMMENT ON COLUMN orders.pit_amount IS
  'Thuế TNCN ước tính của cửa hàng trên subtotal đơn này — chốt lúc tạo đơn theo merchants.pit_rate tại thời điểm đó, trừ luôn vào merchant_payout.';

-- Thân hàm giữ nguyên y hệt hofa-db/60_preorder_manual_confirm.sql, chỉ đổi 2 chỗ: thêm
-- v_vat_rate/v_pit_rate/v_vat_amount/v_pit_amount, và công thức merchant_payout ở INSERT.
