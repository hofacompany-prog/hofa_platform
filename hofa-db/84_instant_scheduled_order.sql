-- Đơn giao ngay (sales_model='instant') giờ cũng đặt trước được ngay ở màn thanh toán app khách
-- (khác tab "Đặt trước"/"Giá sỉ" vốn dùng sales_model='scheduled', đổi cả mã đơn sang prefix
-- riêng — xem 23_order_code_scheduled_prefix.sql). Đơn này VẪN mang mã HF (sales_model không
-- đổi) nhưng "ngủ" — cửa hàng chưa nhận thông báo/chưa thấy đơn trong danh sách — tới đúng lúc
-- còn default_prep_minutes phút nữa là tới scheduled_for thì mới "nổ" cho cửa hàng (báo + bắt
-- đầu thanh trượt xác nhận), y hệt 1 đơn tức thời vừa đặt. Đặt gấp (thời điểm nổ đã tới ngay lúc
-- đặt) thì coi như đơn tức thời luôn, không ngủ — cùng cách xử lý "đặt gấp" đã áp dụng cho tab
-- Đặt trước cũ (xem 49_preorder_gating.sql).
BEGIN;

ALTER TABLE orders ADD COLUMN scheduled_activated_at TIMESTAMPTZ;
COMMENT ON COLUMN orders.scheduled_activated_at IS
  'Chỉ có ý nghĩa khi sales_model=instant VÀ scheduled_for khác NULL (đơn giao ngay đặt trước ở màn thanh toán). NULL = đơn đang "ngủ": cửa hàng chưa được báo/chưa thấy đơn (GET /merchants/:id/orders tự loại các đơn này). Có giá trị = đã "nổ" cho cửa hàng (đúng lúc còn default_prep_minutes phút nữa tới scheduled_for, do sweepDueScheduledInstant set, xem orderOffer.js) hoặc đặt gấp nên coi như tức thời ngay lúc tạo đơn. Vô nghĩa với đơn không đặt trước (scheduled_for NULL) hoặc sales_model=scheduled (dùng preorder_notified_at riêng).';

-- create_order(): thêm nhánh "đơn giao ngay đặt trước" — nếu còn xa (chưa tới ngưỡng
-- default_prep_minutes trước scheduled_for) thì để confirm_sweep_deadline NULL (chưa báo cửa
-- hàng, xem POST /orders chỉ gọi orderOffer.offerOrderToMerchant khi KHÔNG "ngủ"); đặt gấp thì
-- giữ nguyên confirm_sweep_deadline đã tính bình thường + đánh dấu scheduled_activated_at=now()
-- (coi như đã nổ). Thân hàm còn lại giữ nguyên y hệt hofa-db/59_preorder_confirm_now.sql.
