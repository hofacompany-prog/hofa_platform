...
class HofaDriverApp extends ConsumerStatefulWidget {
  const HofaDriverApp({super.key});

  @override
  ConsumerState<HofaDriverApp> createState() => _HofaDriverAppState();
}

class _HofaDriverAppState extends ConsumerState<HofaDriverApp> {
  @override
  void initState() {
    super.initState();
    if (kIsWeb) {
  ...
  Future<void> _checkNativeUpdate() async {
    ...
    // (Navigator của MaterialApp.router chưa kịp gắn) — xác nhận qua debug thật (app Cửa hàng),
    // khiến bước kiểm tra ép cập nhật bị bỏ qua âm thầm mỗi lần. Thử lại tối đa 4s thay vì bỏ
    // cuộc ngay.
