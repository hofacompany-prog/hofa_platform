...
App Cửa hàng HOFA — Android, iOS & Web (PWA). Quản lý sản phẩm/tồn kho, nhận đơn mới (thanh
trượt xác nhận), quản lý nhân viên/chi nhánh, xem ví/thu nhập.
...
push notification (đơn mới) sẽ không hoạt động.

## Thiết lập Firebase (bắt buộc để có push notification trên Android/iOS)

Web đã dùng sẵn project Firebase `hofa-production` — Android/iOS thêm app vào **cùng project
này**, không tạo project mới, để dùng chung 1 nguồn push cho mọi nền tảng.

1. Vào [Firebase Console](https://console.firebase.google.com) → project `hofa-production`.
2. **Thêm app Android**: package name `com.hofa.hofa_store` (khớp `applicationId` trong
   `android/app/build.gradle.kts`) → tải file `google-services.json` → bỏ vào
   `android/app/google-services.json`.
3. **Thêm app iOS**: Bundle ID `com.hofa.hofaStore` (khớp `PRODUCT_BUNDLE_IDENTIFIER` trong
...
- Trả lời bảng câu hỏi Data Safety (Google Play) / App Privacy (App Store) về dữ liệu thu thập
  (vị trí chi nhánh, số điện thoại, thông báo đẩy).
