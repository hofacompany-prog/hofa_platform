...
class ProductRepository {
  ...
  /// Thư viện biến thể mẫu của 1 cửa hàng — xem [VariantTemplate].
  Future<List<VariantTemplate>> variantTemplates(String merchantId) async {
    final list =
        await _api.get('/merchants/$merchantId/variant-templates') as List;
    return list
        .map((e) => VariantTemplate.fromJson(e as Map<String, dynamic>))
        .toList();
  }

  Future<VariantTemplate> variantTemplate(String id) async =>
      VariantTemplate.fromJson(
        await _api.get('/variant-templates/$id') as Map<String, dynamic>,
      );

  Future<VariantTemplate> createVariantTemplate({
    required String merchantId,
    required String name,
    required int price,
    int? comparePrice,
    int? costPrice,
    int? wholesalePrice,
  }) async => VariantTemplate.fromJson(
    await _api.post(
          '/merchants/$merchantId/variant-templates',
          body: {
            'name': name,
            'price': price,
            if (comparePrice != null) 'compare_price': comparePrice,
  ...
  Future<void> updateVariantTemplate(
    String id,
    Map<String, dynamic> data,
  ) async {
    await _api.patch('/variant-templates/$id', body: data);
  }

  Future<void> deleteVariantTemplate(String id) async {
    await _api.delete('/variant-templates/$id');
  }

  Future<WholesaleTier> createVariantTemplateTier({
    ...
    await _api.post(
          '/variant-templates/$templateId/tiers',
          body: {
            'min_quantity': minQuantity,
            if (maxQuantity != null) 'max_quantity': maxQuantity,
            'unit_price': unitPrice,
  ...
  Future<void> updateVariantTemplateTier(
    String id,
    Map<String, dynamic> data,
  ) async {
    await _api.patch('/variant-template-tiers/$id', body: data);
  }

  Future<void> deleteVariantTemplateTier(String id) async {
    await _api.delete('/variant-template-tiers/$id');
  }
