...
class _UserDetailScreenState extends ConsumerState<UserDetailScreen> {
  ...
  Future<void> _editInfo(UserProfile u) async {
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(dob == null ? 'Ngày sinh: chưa có' : 'Ngày sinh: ${formatDateTime(dob!)}'),
                    ),
                    TextButton(
                      onPressed: () async {
    ...
    await _run(() => ref.read(adminRepoProvider).updateUser(u.id, {
          'full_name': nameCtrl.text.trim(),
          'phone': phoneCtrl.text.trim(),
          'email': emailCtrl.text.trim().isEmpty ? null : emailCtrl.text.trim(),
          if (dob != null) 'date_of_birth': dob!.toIso8601String().substring(0, 10),
        }));
  }
  ...
  Future<void> _changeRole(UserProfile u) async {
    ...
    await _run(() => ref.read(adminRepoProvider).setUserRole(u.id, selected).then((_) {}));
  }
    ...
    if (blocking) {
      final ok = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('${u.fullName} sẽ không đăng nhập và đặt hàng được cho tới khi mở lại.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Huỷ')),
            FilledButton(
  ...
  Future<void> _deleteUser(UserProfile u) async {
    ...
    if (ok != true) return;
    await _run(() => ref.read(adminRepoProvider).deleteUser(u.id), goBackAfter: true);
  }
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              children: [
                                CircleAvatar(
                                  ...
                                  child: u.avatarUrl == null
                                      ? Icon(blocked ? Icons.block : Icons.person,
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          const SizedBox(width: 8),
                                          Chip(
                                            label: Text(statusLabels[u.status] ?? u.status),
                                            visualDensity: VisualDensity.compact,
                                            backgroundColor: blocked ? theme.colorScheme.errorContainer : null,
                                          ),
                                        ],
                                      ),
                                    ],
                            ...
                            _row('Email', u.email ?? '—'),
                            _row('Ngày sinh', u.dateOfBirth != null ? formatDateTime(u.dateOfBirth!) : '—'),
                            _row('SĐT xác minh lúc',
                                u.phoneVerifiedAt != null ? formatDateTime(u.phoneVerifiedAt!) : 'Chưa xác minh'),
                            _row('Email xác minh lúc',
                                u.emailVerifiedAt != null ? formatDateTime(u.emailVerifiedAt!) : 'Chưa xác minh'),
                            _row('Đăng nhập gần nhất',
                                u.lastLoginAt != null ? formatDateTime(u.lastLoginAt!) : '—'),
                            _row('Tạo lúc', u.createdAt != null ? formatDateTime(u.createdAt!) : '—'),
                            _row('Cập nhật lúc', u.updatedAt != null ? formatDateTime(u.updatedAt!) : '—'),
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                OutlinedButton.icon(
                                  onPressed: _busy ? null : () => _changeRole(u),
                                  icon: const Icon(Icons.badge_outlined),
                                  label: const Text('Đổi vai trò'),
                                ),
                                OutlinedButton.icon(
                                  onPressed: _busy ? null : () => _toggleBlock(u),
                                  icon: Icon(blocked ? Icons.lock_open : Icons.lock_outline),
                                  label: Text(blocked ? 'Mở khoá tài khoản' : 'Tạm khoá tài khoản'),
                                ),
                              ],
                            ),
                          ],
                    ...
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Địa chỉ đã lưu (${detail.addresses.length})',
                                style: theme.textTheme.titleSmall),
                            const SizedBox(height: 8),
                            if (detail.addresses.isEmpty) const Text('Chưa có địa chỉ nào'),
                            ...detail.addresses.map((a) => _addressTile(a, theme)),
                          ],
                        ),
                      ),
                    ),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('${a.label ?? 'Địa chỉ'} · ${a.recipientName} · ${a.recipientPhone}',
            ...
            if (a.isDefault)
              const Padding(
                ...
                child: Chip(label: Text('Mặc định'), visualDensity: VisualDensity.compact),
              ),
          ],
        ),
      );
